            ...
            <div class="container">
                <div class="row">
                    <div class="col-4">
                        ...
                        <form id="procurarProduto" method="GET" action="ListagemHistoricoEstoque.php">
                        ...
                        if(isset($_GET['datatxt'])){
                            try{
                                ...
                                if($conn){
                                    ...
                                    WHERE data_servico >= ? order by data_servico desc');
                                    $query->execute(array($_GET['datatxt']));
                                    $results=$query->fetchAll(PDO::FETCH_ASSOC);
                                }
                                echo'<table class="table table-striped">   <tr class="textos_cima_label"><th>Produto</th><th>Data</th><th>Quantidade</th><th>Funcionário</th><th>Cliente</th><th>Relatório</th></tr>';
                                foreach($results as $result): ?>
                                    <tr class="textos_cima_label">
                                        ...
                                        <td><a href='../arquivos_Alterar/Frm_Alterar_Produto.php?id=<?php echo $id ?>'> <img src='../../imagens/editar.png' width="50px"> </a></td>
                                    </tr>
                                <?php endforeach;
                                echo'</table>';
    ...
    <script> //script chama php complementar, que faz uma varredura no banco a procura de entidades com nome parecido
     ...
     const elementoProcurado = document.getElementById('dataProcurado');
        ...
        elementoProcurado.addEventListener('change', () => {
            ...
            fetch(` http://localhost/TCC/Telas/phpComplementar/produtosProcuraHistorico.php?datatxt=${value}`)
                .then(response => response.json())
                .then(results => {
                    let resultsHTML = '<table class="table table-striped">   <tr class="textos_cima_label"><th>Produto</th><th>Data</th><th>Quantidade</th><th>Funcionário</th><th>Cliente</th><th>Relatório</th></tr>';
                    results.forEach(item => {
                        resultsHTML += `  
                         <tr class="textos_cima_label">
                            ...
                            <td><a href='../arquivos_Alterar/Frm_Alterar_Produto.php?id=${item.id_produto}'> <img src='../../imagens/editar.png' width="50px"> </a></td>
                        </tr>`;
                    });
                    resultsHTML += '</table>';
