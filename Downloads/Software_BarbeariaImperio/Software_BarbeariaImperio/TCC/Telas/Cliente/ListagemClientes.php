...
<header style="display: flex; justify-content: space-between; align-items: center;">
    ...
    <div class="quadrado-arredondado-geral-listagem-funcAtv">           <!-- div do quadrado preto de fundo -->
        <div class="centro-quadrado-fino">                        <!-- div feita para colocar elementos no meio do quadrado -->
            ...
            <div class="container">
                <div class="row">
                    <div class="col-4">
                        <h1 class="texto_grande">Procurar Cliente</h1>
                 
                        <form id="procurarCliente" method="GET" action="ListagemClientes.php">
                            <input class="form-control" id="clienteProcurado" type="text" name="cliente" placeholder="Nome do Cliente">   
  ...
  echo"<a href='http://localhost/TCC/Telas/Cliente/TelaDecisaoCliente.php'><img src='http://localhost/TCC/imagens/seta.png'></a>";
                    ...
                    <div class="col-8" id="results"><br>
                        ...
                        if(isset($_GET['cliente'])){
                            try{
                                ...
                                echo'<table class="table table-striped">   <tr class="textos_cima_label"><th>Nome</th><th>Sobrenome</th><th>Telefone</th><th>Relatório</th></tr>';
                                foreach($results as $result): ?>
                                    <tr class="textos_cima_label">
                                        <?php  $id=$result['id_Cliente']?>
                                        <td><?= $result['Nome_Cliente']?></td>
                                        <td><?= $result['Sobrenome_Cliente']?></td>
                                        <td><?= $result['Telefone_Cliente']?></td>

                                        <td><a href='../arquivos_Alterar/Frm_Alterar_Cliente.php?id=<?php echo $id ?>'> <img src='../../imagens/editar.png' height='50px'> </a></td>
                                    </tr>
                                <?php endforeach;
                                echo'</table>';
                            ...
                            try{
                                ...
                                $results=[];
                                if($conn){
                                    $query=$conn->prepare('select * from tb_Cliente');
                                    $query->execute();
                                    $results=$query->fetchAll(PDO::FETCH_ASSOC);
                                }
                                echo'<table class="table table-striped">   <tr class="textos_cima_label"><th>Nome</th><th>Sobrenome</th><th>Telefone</th><th>Relatório</th></tr>';
    ...
    <script> //script chama php complementar, que faz uma varredura no banco a procura de entidades com nome parecido
     ...
     const elementoProcurado = document.getElementById('clienteProcurado');
        const resultsElem = document.getElementById('results');
        elementoProcurado.addEventListener('keyup', () => {
            const value = elementoProcurado.value;
           
            fetch(` http://localhost/TCC/Telas/phpComplementar/clientesProcurar.php?cliente=${value}`)
                .then(response => response.json())
                .then(results => {
                    let resultsHTML = '<table class="table table-striped">   <tr class="textos_cima_label"><th>Nome</th><th>Sobrenome</th><th>Telefone</th><th>Relatório</th></tr>';
                    results.forEach(item => {
                        resultsHTML += `  
                         <tr class="textos_cima_label">
                            ...
                            <td>${item.Sobrenome_Cliente}</td>
                            <td>${item.Telefone_Cliente}</td>
                           
                            <td><a href='../arquivos_Alterar/Frm_Alterar_Cliente.php?id=${item.id_Cliente}'> <img src='../../imagens/editar.png' height='50px'> </a></td>
                        </tr>`;
                    });
                    resultsHTML += '</table>';
